...
exports.findAll = (req, res) => {
    ...
    StaticContent.findAll({
      include: [
        {
          model: StaticContentGroup,
          attributes: [['name', 'groupName']] //name renamed as groupName
        }
      ]
    })
    .then((data) => {
...
exports.findOne = (req, res) => {
  ...
  StaticContent.findOne({ where: { id: id }})
  .then((data) => {
...
exports.update = (req, res) => {
  ...
  console.log(req.body);
  const id = req.params.id;
  const find = StaticContent.findOne({ where: { id: id}});
  ...
  if(find)
  {
    StaticContent.update(data, { where: { id: id} })
    .then((data) => {
      res.status(200).send({
...
exports.delete = (req, res) => {
  ...
  if(find)
  {
    StaticContent.destroy({ where: {id: id}})
    .then((data) => {
      res.status(200).send({
...
exports.deleteAll = (req, res) => {
  StaticContent.destroy({where: {}, truncate: false})
  .then((data) => {
